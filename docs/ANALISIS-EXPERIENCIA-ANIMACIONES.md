...
## ExpoJuy 2026 — Predio Ferial Jujuy (La ExpoJuy 2026 se realizará del 9 al 12 de octubre en Ciudad Cultural)
...
**Metodología de esta pasada:** `astro dev` en `localhost:4321`, interacción real (click, type, cambio de selects) más inspección de estilos computados por consola — no solo lectura de código ni confianza ciega en capturas. A mitad de la revisión el panel de navegador quedó oculto del lado del cliente: las capturas dejaron de renderizar contenido (fondo crema sin texto, pese a que `getComputedStyle` confirmaba `opacity: 1` en los mismos elementos) y el viewport reportó `0x0`. Antes de reportar eso como bug, lo descarté cruzando con DOM — era un problema de captura, no de la app. A partir de ahí seguí verificando por ejecución de JavaScript en la página (no por click/type simulados con coordenadas de píxel, que necesitan un viewport real): `el.click()` real sobre botones, inyección de `input` events para inputs controlados por Preact, y lectura de `localStorage`/`network requests`. Es interacción real contra la app corriendo, con una técnica distinta a la visual. La única pantalla que **queda `NO VERIFICADO`** en esta revisión es la **pasada responsive/mobile**, porque evaluar layout responsive sin viewport real no es honesto — ahí me apoyo en las capturas `23-responsive-tablet-768.png` y `24-responsive-mobile-360.png` de la sesión original, no en un chequeo propio fresco.
...
- **`/login` → estado de error:** **confirmo** el hallazgo original (aparece de golpe, sin `animation` ni `transform`), y **matizo algo más importante que la animación**: el formulario no es una simulación de auth — `LoginForm.tsx` llama de verdad a `POST /api/authenticate` contra `PUBLIC_API_BASE_URL` (`http://localhost:8080`, un backend NestJS/JHipster real en `backend/`). Probé login con credenciales incorrectas y `read_network_requests` mostró `net::ERR_CONNECTION_REFUSED` — el backend no está corriendo. Pese a eso, el usuario ve el mismo mensaje genérico *"Usuario o contraseña incorrectos."* que vería con credenciales mal tipeadas. **Esto es más urgente que el shake que proponía el documento original:** antes de animar el error, el mensaje debería distinguir "credenciales inválidas" (401 real) de "servicio no disponible" (falla de red) — sino cualquier corte del backend se le muestra al usuario como si él se hubiera equivocado. Punto a favor: el `<p>` de error ya usa `role="alert"`, así que un lector de pantalla lo anuncia correctamente — ese detalle de accesibilidad está bien resuelto.
- **`/mi-cuenta`:** no pude loguearme de verdad (mismo backend caído), así que inyecté una sesión válida directo en `localStorage` (mismo formato que usa `session.ts`) para ver la vista autenticada. **Confirmo** que es una caja estática sin dinamismo, como decía el documento. **Hallazgo nuevo:** al fallar el refresco de datos contra el backend caído, la pantalla no rompe ni queda en blanco — muestra *"No pudimos actualizar tus datos. Mostrando la última versión guardada."* y sigue mostrando los datos cacheados. Es buen manejo defensivo, vale la pena preservarlo tal cual al animar esta pantalla (la credencial 3D tilt del WOW-006 no debería tapar ni reemplazar ese mensaje de fallback).
- **`/noticias` y artículo:** **confirmo**. Grilla estática de 4 notas, sin `role="progressbar"` ni barra de lectura en el detalle del artículo — coincide con lo que decía el documento.
- **`/404`:** **confirmo**. Pantalla simple, sin animación, con único CTA de vuelta al inicio — coincide con lo que decía el documento, no hay más para agregar acá.
